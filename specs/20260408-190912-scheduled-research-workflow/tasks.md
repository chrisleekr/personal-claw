...
- [x] T002 Add the job definition with `runs-on: ubuntu-latest`, `timeout-minutes: 15`, checkout step using `actions/checkout@v4` with `fetch-depth: 0`, and the `anthropics/claude-code-action@v1` step with `anthropic_api_key: ${{ secrets.ANTHROPIC_API_KEY }}`, `github_token: ${{ secrets.PERSONAL_ACCESS_TOKEN }}`, `allowed_bots: '*'`, and `track_progress: true`
